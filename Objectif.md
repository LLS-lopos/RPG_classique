## Faire un RPG classic

### Style donjon crawler et hack and slash

+ simple :
    - [x] affrontement 1-hero vs 1-ennemi
    - [x] affrontement 1-hero vs groupe ennemie
    - [ ] affrontement groupe-hero vs groupe ennemie
    - [ ] équilibrage du niveau
    - [ ] équilibrage de la difficulté
    - [ ] system d'objet
    - [ ] system d'équipement
    - [ ] systme d'équipe
    - [x] nombre d'ennemi alléatoire
    - [ ] system de gestion d'équipe
    - [ ] ajout d'une minimap
    - [ ] combat aléatoire (simple random)
    - [ ] équilibrage du tour de jeu (qui joue avant l'autre)
    - [ ] équilibrage des classes

+ Faire plus :
    - [ ] system de combat aléatoire (corp a corp, soin, magie)
    - [x] gain d'expérience
    - [x] gain de niveau
    - [ ] system de loute (gain argent/objet)
    - [ ] inventaire (sac)
    - [ ] inventaire (sac individual pour chaque personnage)
    - [ ] system de boutique (achat/ventre)
    - [ ] system de sauvegarde
    - [ ] system de chargement de sauvegarde
    - [ ] system multiple partie
    - [ ] system multijoueur local (PvP)
    - [ ] system multijoueur local (collaboration)
    - [ ] system multijoueur online (PvP)
    - [ ] system multijoueur online (collaboration)
    - [ ] combat aléatoire (via la map)